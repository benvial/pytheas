...
If (incl_flag)
  For i In {2:5}
    istart=1000*i;
    rb~{i}=0.45/(6 - i)*period_x;
    rl~{i}=0.45/(6 - i)*period_y;
    Point(istart)  = { 0  , 0  , hh_L~{i} , lc_layer~{i}};  //Centre du plot élliptique L_4
    Point(istart+1)  = { rb~{i} , 0  , hh_L~{i} , lc_layer~{i}};  //Point du grand axe du plot élliptique  (+X) L_4
    Point(istart+2)  = { 0  , rl~{i} , hh_L~{i} , lc_layer~{i}};  //Point du petit axe du plot élliptique  (+Y) L_4
    Point(istart+3)  = {-rb~{i} , 0  , hh_L~{i} , lc_layer~{i}};  //Point du grand axe du plot élliptique  (-X) L_4
    Point(istart+4)  = { 0  ,-rl~{i} , hh_L~{i} , lc_layer~{i}};  //Point du petit axe du plot élliptique  (-Y) L_4

    Ellipse(istart) = {istart+1,istart,istart+1,istart+2};
    Ellipse(istart+1) = {istart+2,istart,istart+1,istart+3};
    Ellipse(istart+2) = {istart+3,istart,istart+1,istart+4};
    Ellipse(istart+3) = {istart+4,istart,istart+1,istart+1};

    Line Loop(istart) = {istart, istart+1, istart+2, istart+3}; // base plot L_4
    Plane Surface(istart) = {istart}; // base plot L_4

    Extrude {0, 0, thick_L~{i}}{ Surface{istart} ; }
  EndFor
EndIf
...
Line Loop(174) = {16,17,18,19};
Line Loop(175) = {21,22,23,24};
Line Loop(176) = {26,27,28,29};
Line Loop(177) = {31, 32, 33, 34};

If (incl_flag)
  Line Loop(5001) = {5005,5006,5007,5008};
  Plane Surface(10000) = {174, 5001};
  Line Loop(4001) = {4005,4006,4007,4008};
  Plane Surface(10001) = {175, 4001};
  Line Loop(3001) = {3005,3006,3007,3008};
  Plane Surface(10002) = {176, 3001};
  Line Loop(2001) = {2005,2006,2007,2008};
  Plane Surface(10003) = {177, 2001};
  
Else
  Plane Surface(10000) = {174};
  Plane Surface(10001) = {175};
  Plane Surface(10002) = {176};
  Plane Surface(10003) = {177};
EndIf
...
If (incl_flag)
  Plane Surface(200) = {173, 5000};
  Plane Surface(201) = {174, 4000}; 
  Plane Surface(202) = {175, 3000}; 
  Plane Surface(203) = {176, 2000}; 
  
Else
  Plane Surface(200) = {173};
  Plane Surface(201) = {174}; 
  Plane Surface(202) = {175}; 
  Plane Surface(203) = {176}; 
EndIf

Plane Surface(204) = {177};
Plane Surface(205) = {2001};
...
If (incl_flag)
  Surface Loop(10012) = {200, 143, 111, 159, 127, 10000, 5012,5016,5020,5024};
  Surface Loop(10014) = {201, 113, 161, 129, 145, 10001, 4012,4016,4020,4024};
  Surface Loop(10016) = {202, 115, 163, 131, 147, 10002, 3012,3016,3020,3024};
  Surface Loop(10018) = {203, 149, 117, 165, 133, 10003, 2012,2016,2020,2024};
  Surface Loop(10010) = {172, 125, 157, 109, 141, 200, 5000};

Else
  Surface Loop(10012) = {200, 143, 111, 159, 127, 10000};
  Surface Loop(10014) = {201, 113, 161, 129, 145, 10001};
  Surface Loop(10016) = {202, 115, 163, 131, 147, 10002};
  Surface Loop(10018) = {203, 149, 117, 165, 133, 10003};
  Surface Loop(10010) = {172, 125, 157, 109, 141, 200};
EndIf


Surface Loop(10008) = {139, 107, 155, 123, 170, 172};
Volume(10009) = {10008}; //PML_bot
Volume(10011) = {10010}; //L_6 substrate
Volume(10013) = {10012}; //L_5 

Volume(10015) = {10014}; //L_4
Volume(10017) = {10016}; //L_3
Volume(10019) = {10018}; //L_2 
Surface Loop(10020) = {204, 167, 119, 151, 135, 190};
Volume(10021) = {10020}; //L_1 superstrate
...
Physical Volume(5000) = {10017};  // layer L3  (interp)
Physical Volume(6000) = {10019};  // layer L2
Physical Volume(7000) = {10021};  // layer L1
Physical Volume(8000) = {10023};  // PML top
Physical Volume(9000) = {4};  // rod5
Physical Volume(10000) = {3};  // rod4
Physical Volume(11000) = {2};  // rod3
Physical Volume(12000) = {1};  // rod2
